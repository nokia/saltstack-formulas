...
# haproxy_settings = {'elasticsearch': [{'service_port':2400}], 'cassandra': [{'service_port':2444, 'mode':'tcp', 'balancing_mode':'rr'}], 'roles':[{'query':'roles:xx', 'name':'tachyon', 'port':8031, 'service_port':2000}] }
def services():
...
def _services_by_marathon(haproxy_settings, apps):
  return [ {'id': str(app_id), 'services': _as_services(tasks, _get_settings(app_id, haproxy_settings))} for app_id, tasks in apps.iteritems()]
...
def _as_services(tasks, settings):
  ...
  services = [_merge(service_port, setting) for service_port, setting in zip(service_ports, settings)]
